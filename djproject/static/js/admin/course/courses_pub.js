$(function () {
  ...
  let BAIDU_VOD_DOMAIN = 'kc0mpevtkgzz5gz56h6.exp.bcevod.com';	// 百度云VOD域名
  ...
  function getMid(sMediaId){
          CLIENT.listMediaResources()
          .then(function (response) {
            for (var i = 0; i < response.body.media.length; i++) {
              if(response.body.media[i].mediaId === sMediaId){
                  let client = response.body.media[i];
  ...
  $docsBtn.click(function () {
    let sonline_play_url = $('#online-play-url').val();
    if(!sonline_play_url){
      message.showError('请填写在线播放地址！');
      return
    }
    // 判断课程标题是否为空
    ...
    let data = {
      ...
      "online_play_url":sonline_play_url,
